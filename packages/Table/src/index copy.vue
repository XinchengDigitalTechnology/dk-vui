...
const handleScroll = ({ type, scrollTop, scrollLeft, isX, isY }) => {
  if (isY) {
    console.log('scrollTop', scrollTop)
    const header = proxy.$refs.header
    const headerContent = proxy.$refs.headerContent
    const { height } = headerContent.getBoundingClientRect()
    const offset = Math.min(scrollTop, height + 10)
    headerContent.style.transform = `translateY(${-offset + 'px'})`
    header.style.height = `${height - offset}px`
  }
}
  ...
  <div class="vx-table">
    <div ref="header">
      <div ref="headerContent">
        ...
        <div class="vx-table__toolbar">
          <slot name="toolbar_btns" />
        </div>
      </div>
    </div>
    <div class="vx-table__content">
      <vxe-grid ref="gridRef" v-bind="attrs" @scroll="throttleScorll">
