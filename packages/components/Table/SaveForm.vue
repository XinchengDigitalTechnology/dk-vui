...
import { Search, Delete, Top } from "@element-plus/icons-vue"
...
const updateSort = () => {
  if (!proxy.sort) {
    console.error("formConfig.proxy.sort 不能为空")
    return
  }
  proxy.sort({ model_type, list: list.value, formConfig: table.formConfig })
}
...
const open = () => {
  ...
  ElMessageBox.prompt("搜索条件名称", "保存搜索条件", {
    ...
    .then(({ value }) => {
      ...
      proxy
        .save({ model_type, name: value, [mainKey]: 0, conditions, formConfig: table.formConfig })
        .then((res) => {
...
const remove = (id, i) => {
  ...
  proxy
    .remove({ [mainKey]: id, formConfig: table.formConfig })
    .then((res) => {
...
const topPost = (i) => {
  let data = list.value.splice(i, 1)[0]
...
<template>
  <el-dropdown ref="downRef" split-button :placement="placement" trigger="click" type="primary" :hide-on-click="false" popper-class="v-save-search" @click="open" @visible-change="visibleChange">
    ...
    <template #dropdown>
      <div>
        ...
        <el-scrollbar max-height="520px">
          <VueDraggable v-model="list" @update="updateSort" :disabled="!proxy.isSort || (proxy.isSort && filterValue)">
            <div v-for="(d, i) in filterList" :key="i" class="v-save-search-item">
              ...
              <div class="handle">
                <el-button link :loading="loadings['remove' + i]" :icon="Top" @click.stop="topPost(i)" v-if="proxy.isSort"></el-button>
